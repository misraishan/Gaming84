...
export async function optout(
  ...
  if (!user?.isOptedIn) {
    await interaction.reply({
      ephemeral: true,
      content: "Already opted out! Consider opting back in with `/optin`?",
    });
    return;
  }
      ...
      collector.on("collect", async (i: any) => {
        if (i.component.label === "Opt out") {
          ...
          await i.reply({
            ephemeral: true,
            components: [],
            content: "You've successfully opted out & deleted previous data!",
          });
        } else {
          await db.user.upsert({
            where: { id: i.user.id },
          ...
          await i.reply({
            ephemeral: true,
            components: [],
            content: "Thanks for sticking with us!",
          });
        }
      });
    });
