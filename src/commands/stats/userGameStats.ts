...
export async function userGameStats(interaction : ChatInputCommandInteraction<CacheType>) {
    ...
    if (isNumberObject(game)) {
        gameList.forEach((val) => {
            if (val.gameId === game) {
                return interaction.reply(`<@${userId}> has ${convertToReadableTime(val.time)} in ${val.game.name}`);
            }
        })
    } else {
        gameList.forEach((val) => {
    ...
    return interaction.reply(`Could not find <@${userId}>'s playtime for ${game} in our database.\nCheck your spelling and try again?`)
}